      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-gray-50/80 dark:bg-slate-800/50 border-b border-gray-200 dark:border-slate-700">
                    <TableHead className="!text-center text-gray-700 dark:text-slate-300 font-semibold">
                      Employee Code
                    </TableHead>
                    <TableHead className="!text-center text-gray-700 dark:text-slate-300 font-semibold">
                      Name
                ...
                <TableBody>
                  {recentEmployees.map((employee: Employee) => (
                    <TableRow
                      key={employee.id}
                      className="hover:bg-gray-50/50 dark:hover:bg-slate-800/50 border-gray-200 dark:border-slate-700"
                    >
                      <TableCell className="text-center font-medium text-gray-900 dark:text-slate-100">
                        {employee.code}
                      </TableCell>
                      <TableCell className="text-center text-gray-700 dark:text-slate-300">
                        {employee.name}
                      </TableCell>
                      <TableCell className="text-center text-gray-500 dark:text-slate-400">
                        {employee.unit}
                      </TableCell>
                      <TableCell className="text-center text-gray-500 dark:text-slate-400">
                        {employee.phone}
        ...
        <Card className="col-span-1 h-fit bg-white dark:bg-slate-900 border-gray-200 dark:border-slate-700">
          ...
          <CardContent className="flex flex-col space-y-3 pt-6">
            <Button
              variant="outline"
              className="w-full justify-between group cursor-pointer bg-white dark:bg-slate-900 border-blue-200 dark:border-blue-900/50 hover:bg-blue-50 dark:hover:bg-slate-800 text-blue-600 dark:text-blue-400"
              onClick={() => navigate("/employees")}
            >
              <span className="flex items-center">
                <Users className="mr-2 h-4 w-4 text-blue-600 dark:text-blue-400 transition-colors" />
                View Employees
              </span>
              <ChevronRight className="h-4 w-4 text-blue-600 dark:text-blue-400 transition-colors" />
            </Button>
            <Button
              variant="outline"
              className="w-full justify-between group cursor-pointer bg-white dark:bg-slate-900 border-purple-200 dark:border-purple-900/50 hover:bg-purple-50 dark:hover:bg-slate-800 text-purple-600 dark:text-purple-400"
              onClick={() => navigate("/reports")}
            >
              <span className="flex items-center">
                <FileText className="mr-2 h-4 w-4 text-purple-600 dark:text-purple-400 transition-colors" />
                Generate Report
              </span>
              <ChevronRight className="h-4 w-4 text-purple-600 dark:text-purple-400 transition-colors" />
            </Button>
          </CardContent>
        </Card>
      </div>
