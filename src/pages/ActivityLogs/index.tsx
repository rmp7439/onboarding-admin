...
export default function ActivityLogs() {
  const [filters, setFilters] = useState({ page: 1, limit: 20, search: "", action: "" });
  ...
  const getActionBadge = (action: string) => {
    ...
    if (action.includes("DELETED") || action.includes("DISABLED")) return <Badge variant="destructive">{action}</Badge>;
    ...
    <div className="space-y-6 pb-8">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-500">Immutable audit trail of administrative actions.</p>
        </div>
      </div>
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-slate-50/80">
              ...
              <TableHead className="font-semibold whitespace-nowrap">Actor</TableHead>
              <TableHead className="font-semibold whitespace-nowrap">Action</TableHead>
              <TableHead className="font-semibold whitespace-nowrap">Target</TableHead>
              <TableHead className="font-semibold text-center whitespace-nowrap">Changes</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {data?.logs.length === 0 ? (
              <TableRow><TableCell colSpan={5} className="text-center py-10 text-gray-500">No logs found.</TableCell></TableRow>
            ) : (
                ...
                <TableRow key={log.id} className="hover:bg-slate-50/60">
                  ...
                  <TableCell className="py-4 text-center">
                    {log.changes && log.changes.length > 0 ? (
                      <Button variant="ghost" size="sm" onClick={() => setSelectedChanges(log.changes)} className="text-blue-600">
      ...
      <Dialog open={!!selectedChanges} onOpenChange={() => setSelectedChanges(null)}>
        <DialogHeader><DialogTitle>Field Changes</DialogTitle></DialogHeader>
        <DialogContent className="max-h-[60vh] overflow-y-auto">
          <div className="space-y-4">
            {selectedChanges?.map((change: any, i: number) => (
              <div key={i} className="bg-gray-50 p-3 rounded border border-gray-200">
                <div className="font-semibold text-gray-800 text-sm mb-2 uppercase">{change.field}</div>
