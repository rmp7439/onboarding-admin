...
export default function Reports() {
  ...
  const { data: reportEmployees = [], isLoading } =
    useReportEmployees(appliedFilters);
  ...
  const handleDownloadAllPdfs = async () => {
    if (!hasValidFilter) {
      toast(
        "Please apply a filter before exporting individual reports.",
        "info",
      );
      return;
    }
    ...
    try {
      ...
      toast(
        `Successfully downloaded ${reportEmployees.length} PDFs.`,
        "success",
      );
    } catch (error) {
  ...
  const handleBulkPdf = () => {
    ...
    if (reportEmployees.length === 1) {
      toast(
        "Bulk PDF is available only when multiple employees are returned. Use Individual PDF instead.",
        "info",
      );
      return;
    }
